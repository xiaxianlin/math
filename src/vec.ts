...
export function size(v: VecType): number {
    let sum = v.reduce((total, item) => total + item * item, 0)
    return Math.sqrt(sum)
}

export function add(v1: VecType, v2: VecType): VecType {
    validLength(v1, v2)
    return v1.map((item, index) => item + v2[index])
}

export function substract(v1: VecType, v2: VecType): VecType {
    validLength(v1, v2)
    return v1.map((item, index) => item - v2[index])
}

export function multi(v: VecType, k: number): VecType {
    return v.map((item, index) => item * k)
}

export function normalize(v: VecType): VecType {
    return multi(v, 1 / size(v))
}

export function dot(v1: VecType, v2: VecType): number {
    validLength(v1, v2)
    return v1.reduce((total, item, index) => total + item * v2[index], 0)
}

export function theta(v1: VecType, v2: VecType): number {
    validLength(v1, v2)
