...
class Quaternion {
    ...
    public constructor(w: number = 1.0, x: number = 0.0, y: number = 0.0, z: number = 0.0) {
        this.w = w
        this.x = x
        this.y = y
        this.z = z
     ...
     * 球面线性插值
     */
    public slerp(q1: Quaternion, t: number) {
        let q0 = this.clone()
        // 检查出界的参数，如果检查到，返回边界点
        if (t <= 0.0) return q0
        if (t >= 1.0) return q1
     ...
     */
    public setToRotateObjectToInertial(orientation: EulerAngles) {
     ...
     */
    public setToRotateInertialToObject(orientation: EulerAngles) {
        let [sh, ch] = sinCos(orientation.heading * 0.5)
        let [sp, cp] = sinCos(orientation.pitch * 0.5)
        let [sb, cb] = sinCos(orientation.bank * 0.5)
        this.w = ch * cp * cb + sh * sp * sb
