...
class AABB3 {
    ...
    public static intersectAABBs(box1: AABB3, box2: AABB3, boxIntersect: AABB3) {
     ...
     */
    public static intersectMovingAABB(stationaryBox: AABB3, movingBox: AABB3, d: Vector3) {
     ...
     * 变换矩形边界框，计算新的AABB
     */
    public setToTransformedBox(box: AABB3, m: Matrix4x3) {
        if (box.isEmpty()) {
            this.empty()
            return
        }
     ...
     * 计算最近距离的矩形边界框上的点
     */
    public closestPointTo(p: Vector3) {
        let { x, y, z } = p
        if (x < this.min.x) {
            x = this.min.x
     ...
     * 检测是否与球相交
     */
    public intersectSphere(center: Vector3, radius: number) {
     ...
     * 检测是否与射线相交
     */
    public intersectRay(rayOrg: Vector3, rayDelta: Vector3, intersectPoint: Vector3) {
        let inside = true
        // 检查点在矩形边界框内的情况，并计算到每个面的距离
        let xt: number, xn: number
     ...
     * 判断矩形边界框在平面的哪一面
     */
    public classifyPlane(n: Vector3, d: number) {
     ...
     * 检测是否与平面相交
     */
    public intersectPlane(n: Vector3, planeD: number, dir: Vector3) {
